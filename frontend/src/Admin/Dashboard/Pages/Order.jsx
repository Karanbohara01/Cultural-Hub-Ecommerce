...
const AdminOrderPage = () => {
    ...
    const fetchOrders = async () => {
        try {
            const response = await fetch('http://localhost:8787/order/getAll');
            if (!response.ok) {
                throw new Error('Failed to fetch orders');
            }
            const data = await response.json();
    ...
    const updateOrderStatus = async (orderId, newStatus) => {
        ...
        try {
            const response = await fetch(`http://localhost:8787/order/update/${orderId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const deleteOrder = async (orderId) => {
        if (window.confirm("Are you sure you want to delete this order?")) {
            try {
                const response = await fetch(`http://localhost:8787/order/delete/${orderId}`, {
                    method: 'DELETE',
                });
    ...
    return (
        <>
            ...
            <div style={{ marginLeft: 650, marginTop: 80, marginBottom: -200, display: "flex", alignItems: 'center', justifyContent: 'right' }}>
                <DigitalClock />
            </div>
